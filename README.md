...
面向外贸装柜场景的轻量计算网站。选择单个柜型，录入散箱或整托货物后，每次计算返回一个方案，结果页提供“装载率优先、重心稳妥、易操作”三个优化目标偏好开关，切换目标即重新计算；同时提供 3D、俯视、侧视、柜门和分层布局图。
...
- 三个目标共同遵循三条基本原则：底层先铺满再叠高；只有同规格参数的货物才允许叠放；上层集中到柜长中部而不是分散到两边。
- 每个方案附带平移归一几何指纹（`layout_fingerprint`，12 位哈希）：整体平移、装载步骤编号或实例编号变化不影响指纹，逐件位置或朝向变化才会改变。切换优化目标后，若新方案与上一方案指纹相同，结果页会提示"布局几何相同"——个别订单形态（如完全不可叠的少量整托、柜子装不下剩余无法重排等）下两个目标会收敛到同一排布，这是货物本身决定的，界面会如实披露而不是悄悄展示两张一样的图。
- 结果页在非「重心稳妥」目标下前后重量偏差较大时显示偏柜警告，并可一键切换到「重心稳妥」重新计算。
