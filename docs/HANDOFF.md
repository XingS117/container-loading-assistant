...
- 混装订单的布局策略为**分层铺满（floor-layer-first）**：第 1 层把托盘与散箱单件用 2D 装箱铺满整个柜底（贴壁、密集，面积覆盖约 90%+），放不下的散箱单件按"柱高"叠到第 1 层同 SKU 位置正上方（同 footprint → 100% 完整支撑），每位置叠高层数受栈容量（max_layers/高度/承重）约束；三方案统一。散箱不再上托到托盘顶面（分层铺满优先），易碎散箱不参与叠放。
