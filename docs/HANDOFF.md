...
- AI 不作为坐标计算器或硬依赖，只推荐 SKU 顺序、合法朝向和行组策略；
...
- 装柜响应新增 `ai_strategy.applied` 和 `ai_strategy.row_groups`，只有最终安全方案完整遵循有效建议时才标记为已采纳；
  前端会显示实际采纳的行组数量，避免把“模型请求成功”误报成“布局已采用”。
...
- AI 返回的是策略，不是坐标。解析器兼容标准字符串、结构化内容片段、Markdown 代码块、说明文字中的 JSON、工具参数和 `reasoning_content`；提供商不支持 `response_format` 时会自动无该参数重试一次。
