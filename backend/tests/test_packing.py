...
def test_valid_locked_layout_runs_local_recalculation():
    request = PackRequest.model_validate({
        "container": {"id": "small", "name": "测试柜", "inner_length_mm": 1000, "inner_width_mm": 1000, "inner_height_mm": 1000, "door_width_mm": 1000, "door_height_mm": 1000, "max_payload_g": 1000000},
    ...
    response = pack_order(request)
    assert all(solution.placements[0].x_mm == 0 for solution in response.solutions)
    assert all(solution.loaded_counts["a"] == 1 for solution in response.solutions)
